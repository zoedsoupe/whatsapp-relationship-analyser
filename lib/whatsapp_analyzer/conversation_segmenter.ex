defmodule WhatsAppAnalyzer.ConversationSegmenter do
  ...
  def segment_conversations(df) do
    if DF.n_rows(df) == 0 do
      ...
      conversation_ids
      |> Enum.map(fn conv_id ->
        conv_df = DF.filter_with(df, fn rows ->
  ...
  defp build_segment_metadata(conv_df) do
    ...
    %{
      ...
      summary: summary,
      text_summary: nil  # Optional ML summary, populated on-demand
    }
  end

  defp summarize_metrics(conv_df) do
    avg_message_length = if DF.n_rows(conv_df) > 0 do
    ...
    total_words = if DF.n_rows(conv_df) > 0 do
      conv_df["word_count"]
      |> S.sum()
    else
      0
